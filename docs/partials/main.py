def define_env(env):
    """MkDocs Macros plugin entrypoint: small safe component macros used across the docs.

    Exposes macros:
    - thesis_card(title, takeaway, refs)
    - caution_callout(heading, body)
    - protocol_steps(steps)
    - figure_with_caption(src, caption, id)
    - myth_science_pair(myth, insight)
    - glossary_term(term)
    """
...
def define_env(env):
    ...
    def protocol_steps(steps=None):
        ...
        return f"<ol data-component='protocol-steps'>{items}</ol>"
    def figure_with_caption(src="", caption="", id=""):
        return f"<figure id='{id}'><img src='{src}' alt='{caption}' /><figcaption>{caption}</figcaption></figure>"
    env.macro(thesis_card)
    env.macro(caution_callout)
