...
import { mainNavLinks, protocolLinks } from '../data/navLinks'
      ...
      <div className="max-w-6xl mx-auto flex items-center gap-6 py-3 px-4">
        ...
        <ul className="ml-auto hidden md:flex gap-4 items-center">
          {mainNavLinks.map(link => (
            <li key={link.label}>
              {link.type === 'internal' ? (
                <Link className="nav-link text-sm font-medium" to={link.to}>{link.label}</Link>
              ) : (
                <a className="nav-link text-sm font-medium" href={link.href}>{link.label}</a>
              )}
            </li>
          ))}
            ...
            <ul className={`absolute right-0 mt-3 w-56 bg-white border border-border rounded-lg shadow-lg p-3 ${open ? 'block' : 'hidden'}`}>
              {protocolLinks.map(item => (
                <li key={item.href} className="p-2">
                  <a className="nav-dropdown__link" href={item.href}>{item.label}</a>
                </li>
              ))}
            </ul>
          </li>
